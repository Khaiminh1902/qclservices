...
import fetch, { RequestInit } from "node-fetch"; // Import RequestInit from node-fetch
...
async function fetchWithRetry(
  url: string,
  options: RequestInit, // Use node-fetch's RequestInit
  retries: number = 3,
...
export async function POST(request: Request) {
  try {
    ...
    if (data.success && data.data?.url) {
      ...
      console.error(
        `ImgBB API response indicated failure: Code ${errorCode}, Message: ${errorMessage}`,
        data
      );
      throw new Error(
        `Failed to upload image to ImgBB: ${errorMessage} (Code: ${errorCode})`
      );
    }
  } catch (error) {
